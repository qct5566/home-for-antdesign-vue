    ...
    <div class="grzx-wrap">
      ...
      <div class="grzx-wrap-main">
        <div class="grzx-wrap-panel-inf">
          <span>共{{ pageInfo.total }}个数据大屏</span>
          <div class="right">
            <a-input-search
              v-model="search.name"
              @search="doSearch"
              placeholder="输入数据大屏名称"
              style="width: 240px" />
          </div>
        </div>
        <ul class="sjdp-sceens">
          <li v-for="(scr, index) in dataList" :key="index">
            <div class="title ellipsis click" :title="scr.name" @click="openTab(scr)">{{ scr.name }}</div>
            <div class="img click" @click="openTab(scr)">
...
<script>
import { getDataScreenClassifyAll } from '@/api/modules/dataScreenClassifyService'
import { getDataScreenAll, updateDataScreen } from '@/api/modules/dataScreenService'
import { addDataScreenStar, deleteDataScreenStar } from '@/api/modules/dataScreenStarService'
export default {
  data () {
    return {
      classifyData: [],
  ...
  methods: {
    ...
    getTreeData () {
      const _this = this
      getDataScreenClassifyAll().then(res => {
    ...
    getDataList () {
      ...
      getDataScreenAll(parameter).then(res => {
    ...
    starItem (item) {
      if (this.userId.length > 0) {
        if (item.starId != null) {
          deleteDataScreenStar({ 'ids[]': item.starId }).then(res => {
            item.starId = null
          })
        } else {
          addDataScreenStar({ dataScreenId: item.id, userId: this.userId }).then(res => {
    ...
    openTab (item) {
      ...
      updateDataScreen({ id: item.id, pageView: item.pageView })
